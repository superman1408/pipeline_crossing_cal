...
"background-color: rgb(11, 120, 130);")
        ...
        self.gridLayout_17.addLayout(self.horizontalLayout_6, 1, 0, 1, 1)
        ...
        self.gridLayout_17.addWidget(self.frame, 2, 0, 1, 1)
        ...
        self.gridLayout_20.addWidget(self.lineEdit_37, 0, 1, 1, 1)
        ...
        self.gridLayout_17.addWidget(self.frame_6, 3, 0, 1, 1)
        ...
        self.gridLayout_17.addWidget(self.frame_2, 4, 0, 1, 2)
        ...
        self.gridLayout_17.addWidget(self.frame_7, 5, 0, 1, 1)
        ...
        self.gridLayout_17.addWidget(self.frame_3, 6, 0, 1, 1)
        ...
        self.gridLayout_17.addWidget(self.frame_5, 7, 0, 1, 1)
...
"color: rgb(0, 0, 0);")
        ...
        self.verticalLayout_23.addWidget(self.EarthLoadExcavationFactor_2)
        ...
        self.verticalLayout_24.addWidget(self.EarthLoadExcavationFactor_3)
        ...
        self.gridLayout_17.addItem(spacerItem33, 9, 1, 1, 1)
        ...
        # Connect the SUBMIT button to the calculation function
        self.pushButton.clicked.connect(self.calculate_pipeline_stresses)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.Codes_and_standards_2.setText(_translate("MainWindow", "Bored Diametrer:")) # This is likely a label for the combobox below it, which controls bored diameter calculation
        self.comboBox_2.setItemText(0, _translate("MainWindow", "Considered")) # Option to consider bored diameter
        self.comboBox_2.setItemText(1, _translate("MainWindow", "Not Considered")) # Option to not consider bored diameter
    ...
    # Method to handle calculations and UI updates
    def calculate_pipeline_stresses(self):
        try:
            # 1. Get Input Values from UI
            # Convert text from QLineEdit to float. Use .strip() and default to 0.0 or raise error if empty
            # Mapping based on retranslateUi labels and your calculation code
            Pipe_Outside_Diameter = float(self.lineEdit.text()) # D
            Pipe_Wall_Thickness = float(self.lineEdit_2.text()) # t
            Specified_Minimum_Yield_Strength = float(self.lineEdit_3.text()) # SMYS
            Depth_of_Cover = float(self.lineEdit_5.text()) # H
            Corrosion_Allowance = float(self.lineEdit_4.text()) # CA
            Operating_Pressure = float(self.lineEdit_11.text()) # p (in bar, convert to MPa later)
            Impact_Factor = float(self.lineEdit_12.text()) # Fi
            Design_Factor = float(self.lineEdit_13.text()) # F
            Longitudinal_Joint_Factor = float(self.lineEdit_14.text()) # E
            Installation_Temperature = float(self.lineEdit_15.text()) # T1
            Operating_Temperature = float(self.lineEdit_16.text()) # T2
            # Design_Wheel_Load_From_Single_Axle = float(self.lineEdit_17.text()) # Ps (not used in provided calculations)
            Design_Wheel_Load_From_Tandem_Axle = float(self.lineEdit_18.text()) # Pt
            Youngs_Modulus = float(self.lineEdit_19.text()) # Es
            Poissons_Ratio = float(self.lineEdit_20.text()) # mu
            Coefficient_of_Thermal_Expansion = float(self.lineEdit_37.text()) # alpha_T
            Earth_Load_Stiffness_Factor = float(self.lineEdit_21.text()) # KHe
            Earth_Load_Burial_Factor = float(self.lineEdit_22.text()) # Be
            Earth_Load_Excavation_Factor = float(self.lineEdit_23.text()) # Ee
            Stiffness_Factor_KHh = float(self.lineEdit_27.text()) # KHh
            Geometry_Factor_GHh = float(self.lineEdit_25.text()) # GHh
            Stiffness_Factor_KLh = float(self.lineEdit_29.text()) # KLh
            Geometry_Factor_GLh = float(self.lineEdit_28.text()) # GLh
            Road_Axle_Configuration_Factor = float(self.lineEdit_26.text()) # L
            Road_Pavement_Type_Factor = float(self.lineEdit_24.text()) # R
            Fatigue_Endurance_Limit_of_Girth_yield = float(self.lineEdit_31.text()) # SFG
            Fatigue_Endurance_Limit_of_Longitudinal_Weld = float(self.lineEdit_32.text()) # SFL
            Soil_Unit_Weight = float(self.lineEdit_8.text()) # gamma

            # Convert Operating_Pressure from bar to MPa (1 bar = 0.1 MPa)
            Operating_Pressure_MPa = Operating_Pressure * 0.1

            # 2. Perform Calculations
            # Pipe_Wall_Thickness_Including_CA
            Pipe_Wall_Thickness_Including_CA = (Pipe_Wall_Thickness - Corrosion_Allowance)
            self.lineEdit_6.setText(f"{Pipe_Wall_Thickness_Including_CA:.3f}") # Update UI for t_w

            # Bored Diameter (based on combobox selection: Considered/Not Considered)
            consider_bored_diameter = (self.comboBox_2.currentText() == "Considered")
            if consider_bored_diameter:
                Bored_Diameter = Pipe_Outside_Diameter + 51
            else:
                Bored_Diameter = Pipe_Outside_Diameter
            self.lineEdit_7.setText(f"{Bored_Diameter:.3f}") # Update UI for B_d

            # Intermediate ratios (not directly displayed in your UI, but good to keep if needed)
            ...
            Ratio_of_pipe_dept_and_bore_diameter = (Depth_of_Cover * 1000) / Bored_Diameter # Depth_of_Cover is in meters, convert to mm
            Ratio_of_pipe_dept_and_bore_diameter = round(Ratio_of_pipe_dept_and_bore_diameter, 3)

            # Applied Design Surface Pressure
            Applied_Design_Surface_Pressure = round(Design_Wheel_Load_From_Tandem_Axle / 0.093) / 1000 # Result in MPa
            # If you have a lineEdit for this, update it here.

            # A) Barlow Stress
            # Using Operating_Pressure_MPa (converted from bar)
            Barlow_Stress = (Operating_Pressure_MPa * (Pipe_Outside_Diameter - Pipe_Wall_Thickness_Including_CA)) / (2 * Pipe_Wall_Thickness_Including_CA)
            self.lineEdit_33.setText(f"{Barlow_Stress:.3f}") # Update UI for Barlow Stress (S_Hi)
            ...
            if Barlow_Stress <= F_E_SMYS:
                self.radioButton_7.setChecked(True) # SAFE
                self.radioButton_8.setChecked(False)
            else:
                self.radioButton_7.setChecked(False)
                self.radioButton_8.setChecked(True) # NOT SAFE

            # B) Circumferential Stress due to Earth Load
            # Convert Soil_Unit_Weight from kN/m³ to MPa (if needed for consistency, 1 kN/m³ = 0.001 MPa/mm, or check formula derivation)
            # Assuming Soil_Unit_Weight is already in a compatible unit for MPa output
            ...
            self.lineEdit_35.setText(f"{Stress_due_to_Earth_Load:.3f}") # Update UI for Stress Due to Earth Load (S_He)
            ...
            self.lineEdit_34.setText(f"{Cyclic_Circumferential_Stress:.3f}") # Update UI for Cyclic Circumferential Stress (ΔS_Hh)
            ...
            self.lineEdit_36.setText(f"{Cyclic_Longitudinal_Stress:.3f}") # Update UI for Cyclic Longitudinal Stress (ΔS_Lh)
            ...
            Radial_Stress = 0.0 # As per your code, Radial_Stress is always 0.0

            self.lineEdit_38.setText(f"{Circumferential:.3f}") # Update UI for Circumferential Stress (S1)
            self.lineEdit_39.setText(f"{Longitudinal:.3f}") # Update UI for Longitudinal Stress (S2)
            self.lineEdit_30.setText(f"{Radial_Stress:.3f}") # Update UI for Radial Stress (S3)

            # E) Principle Stress (Effective Stress)
            effective_stress = math.sqrt(0.5 * ((Circumferential - Longitudinal)**2 + \
                                                (Longitudinal - Radial_Stress)**2 + \
                                                (Radial_Stress - Circumferential)**2))
            self.lineEdit_40.setText(f"{effective_stress:.3f}") # Update UI for Effective Stress (S_eff)

            # Principle Stress Criteria Check
            if effective_stress <= Specified_Minimum_Yield_Strength * Design_Factor:
                self.radioButton_5.setChecked(True) # SAFE
                self.radioButton_6.setChecked(False)
            else:
                self.radioButton_5.setChecked(False)
                self.radioButton_6.setChecked(True) # NOT SAFE

            # F) Fatigue Check (Girth Welds)
            if Cyclic_Longitudinal_Stress <= Fatigue_Endurance_Limit_of_Girth_yield * Design_Factor:
                self.radioButton.setChecked(True) # SAFE
                self.radioButton_2.setChecked(False)
            else:
                self.radioButton.setChecked(False)
                self.radioButton_2.setChecked(True) # NOT SAFE

            # F) Fatigue Check (Longitudinal Welds)
            if Cyclic_Circumferential_Stress <= Fatigue_Endurance_Limit_of_Longitudinal_Weld * Design_Factor:
                self.radioButton_3.setChecked(True) # SAFE
                self.radioButton_4.setChecked(False)
            else:
                self.radioButton_3.setChecked(False)
                self.radioButton_4.setChecked(True) # NOT SAFE
            ...
            msg_box.setInformativeText("Please ensure all input fields contain valid numerical values. (e.g., '100.5' instead of 'abc')")
            ...
            msg_box.setInformativeText("A division by zero occurred. Check Pipe Wall Thickness Including CA, it might be zero or too small.")
            ...
            msg_box.setInformativeText(f"An unexpected error occurred during calculation: {e}\nPlease check your inputs and formulas.")
            msg_box.setWindowTitle("Calculation Error")
            msg_box.exec_()

# ... (rest of your Ui_MainWindow class code) ...

    # Method to handle calculations and UI updates
    def calculate_pipeline_stresses(self):
        try:
            # 1. Get Input Values from UI
            print("\n--- Reading Input Values ---") # Added print for clarity

            Pipe_Outside_Diameter = float(self.lineEdit.text()) # D
            print(f"Pipe_Outside_Diameter (D): {Pipe_Outside_Diameter} mm")

            Pipe_Wall_Thickness = float(self.lineEdit_2.text()) # t
            print(f"Pipe_Wall_Thickness (t): {Pipe_Wall_Thickness} mm")

            Specified_Minimum_Yield_Strength = float(self.lineEdit_3.text()) # SMYS
            print(f"Specified_Minimum_Yield_Strength (SMYS): {Specified_Minimum_Yield_Strength} MPa")

            Depth_of_Cover = float(self.lineEdit_5.text()) # H
            print(f"Depth_of_Cover (H): {Depth_of_Cover} m")

            Corrosion_Allowance = float(self.lineEdit_4.text()) # CA
            print(f"Corrosion_Allowance (CA): {Corrosion_Allowance} mm")

            Operating_Pressure = float(self.lineEdit_11.text()) # p
            print(f"Operating_Pressure (p): {Operating_Pressure} bar")

            Impact_Factor = float(self.lineEdit_12.text()) # Fi
            print(f"Impact_Factor (Fi): {Impact_Factor}")

            Design_Factor = float(self.lineEdit_13.text()) # F
            print(f"Design_Factor (F): {Design_Factor}")

            Longitudinal_Joint_Factor = float(self.lineEdit_14.text()) # E
            print(f"Longitudinal_Joint_Factor (E): {Longitudinal_Joint_Factor}")

            Installation_Temperature = float(self.lineEdit_15.text()) # T1
            print(f"Installation_Temperature (T1): {Installation_Temperature} °C")

            Operating_Temperature = float(self.lineEdit_16.text()) # T2
            print(f"Operating_Temperature (T2): {Operating_Temperature} °C")

            Design_Wheel_Load_From_Tandem_Axle = float(self.lineEdit_18.text()) # Pt
            print(f"Design_Wheel_Load_From_Tandem_Axle (Pt): {Design_Wheel_Load_From_Tandem_Axle} kN")

            Youngs_Modulus = float(self.lineEdit_19.text()) # Es
            print(f"Youngs_Modulus (Es): {Youngs_Modulus} MPa")

            Poissons_Ratio = float(self.lineEdit_20.text()) # mu
            print(f"Poissons_Ratio (mu): {Poissons_Ratio}")

            Coefficient_of_Thermal_Expansion = float(self.lineEdit_37.text()) # alpha_T
            print(f"Coefficient_of_Thermal_Expansion (alpha_T): {Coefficient_of_Thermal_Expansion} 1/°C")

            Earth_Load_Stiffness_Factor = float(self.lineEdit_21.text()) # KHe
            print(f"Earth_Load_Stiffness_Factor (KHe): {Earth_Load_Stiffness_Factor}")

            Earth_Load_Burial_Factor = float(self.lineEdit_22.text()) # Be
            print(f"Earth_Load_Burial_Factor (Be): {Earth_Load_Burial_Factor}")

            Earth_Load_Excavation_Factor = float(self.lineEdit_23.text()) # Ee
            print(f"Earth_Load_Excavation_Factor (Ee): {Earth_Load_Excavation_Factor}")

            Stiffness_Factor_KHh = float(self.lineEdit_27.text()) # KHh
            print(f"Stiffness_Factor_KHh (KHh): {Stiffness_Factor_KHh}")

            Geometry_Factor_GHh = float(self.lineEdit_25.text()) # GHh
            print(f"Geometry_Factor_GHh (GHh): {Geometry_Factor_GHh}")

            Stiffness_Factor_KLh = float(self.lineEdit_29.text()) # KLh
            print(f"Stiffness_Factor_KLh (KLh): {Stiffness_Factor_KLh}")

            Geometry_Factor_GLh = float(self.lineEdit_28.text()) # GLh
            print(f"Geometry_Factor_GLh (GLh): {Geometry_Factor_GLh}")

            Road_Axle_Configuration_Factor = float(self.lineEdit_26.text()) # L
            print(f"Road_Axle_Configuration_Factor (L): {Road_Axle_Configuration_Factor}")

            Road_Pavement_Type_Factor = float(self.lineEdit_24.text()) # R
            print(f"Road_Pavement_Type_Factor (R): {Road_Pavement_Type_Factor}")

            Fatigue_Endurance_Limit_of_Girth_yield = float(self.lineEdit_31.text()) # SFG
            print(f"Fatigue_Endurance_Limit_of_Girth_yield (SFG): {Fatigue_Endurance_Limit_of_Girth_yield} MPa")

            Fatigue_Endurance_Limit_of_Longitudinal_Weld = float(self.lineEdit_32.text()) # SFL
            print(f"Fatigue_Endurance_Limit_of_Longitudinal_Weld (SFL): {Fatigue_Endurance_Limit_of_Longitudinal_Weld} MPa")

            Soil_Unit_Weight = float(self.lineEdit_8.text()) # gamma
            print(f"Soil_Unit_Weight (gamma): {Soil_Unit_Weight} kN/m³")

            # Also print the selected combobox values
            print(f"Selected Pipe Type: {self.PipeType_combobox.currentText()}")
            print(f"Selected Soil Type: {self.comboBox_3.currentText()}")
            print(f"Selected Bored Diameter Option: {self.comboBox_2.currentText()}")
            print(f"Selected Steel Grade: {self.comboBox.currentText()}")
            print(f"Selected Codes & Standards: {self.comboBox_4.currentText()}")

            # 2. Perform Calculations
            print("\n--- Performing Calculations ---") # Added print for clarity

            Operating_Pressure_MPa = Operating_Pressure * 0.1
            print(f"Operating_Pressure (converted to MPa): {Operating_Pressure_MPa} MPa")

            # Pipe_Wall_Thickness_Including_CA
            Pipe_Wall_Thickness_Including_CA = (Pipe_Wall_Thickness - Corrosion_Allowance)
            print(f"Pipe_Wall_Thickness_Including_CA (t_w): {Pipe_Wall_Thickness_Including_CA} mm")
            self.lineEdit_6.setText(f"{Pipe_Wall_Thickness_Including_CA:.3f}") # Update UI for t_w
            ...
            if consider_bored_diameter:
                ...
                Bored_Diameter = Pipe_Outside_Diameter
            print(f"Bored_Diameter (Bd): {Bored_Diameter} mm")
            self.lineEdit_7.setText(f"{Bored_Diameter:.3f}") # Update UI for B_d

            # ... (rest of your calculation logic and UI updates) ...

            Thickness_to_diameter_ratio = (Pipe_Wall_Thickness_Including_CA / Pipe_Outside_Diameter)
            Thickness_to_diameter_ratio = round(Thickness_to_diameter_ratio, 5)
            print(f"Thickness_to_diameter_ratio: {Thickness_to_diameter_ratio}")

            Ratio_of_bore_diameter_and_pipe_diameter = (Bored_Diameter / Pipe_Outside_Diameter)
            Ratio_of_bore_diameter_and_pipe_diameter = round(Ratio_of_bore_diameter_and_pipe_diameter, 2)
            print(f"Ratio_of_bore_diameter_and_pipe_diameter: {Ratio_of_bore_diameter_and_pipe_diameter}")

            Ratio_of_pipe_dept_and_bore_diameter = (Depth_of_Cover * 1000) / Bored_Diameter # Depth_of_Cover is in meters, convert to mm
            Ratio_of_pipe_dept_and_bore_diameter = round(Ratio_of_pipe_dept_and_bore_diameter, 3)
            print(f"Ratio_of_pipe_dept_and_bore_diameter: {Ratio_of_pipe_dept_and_bore_diameter}")

            Applied_Design_Surface_Pressure = round(Design_Wheel_Load_From_Tandem_Axle / 0.093) / 1000 # Result in MPa
            print(f"Applied_Design_Surface_Pressure: {Applied_Design_Surface_Pressure} MPa")


            # A) Barlow Stress
            Barlow_Stress = (Operating_Pressure_MPa * (Pipe_Outside_Diameter - Pipe_Wall_Thickness_Including_CA)) / (2 * Pipe_Wall_Thickness_Including_CA)
            print(f"Barlow_Stress (S_Hi): {Barlow_Stress:.3f} MPa")
            self.lineEdit_33.setText(f"{Barlow_Stress:.3f}") # Update UI for Barlow Stress (S_Hi)

            F_E_SMYS = (Design_Factor * Longitudinal_Joint_Factor * Specified_Minimum_Yield_Strength)
            print(f"F_E_SMYS: {F_E_SMYS:.3f} MPa")
            ...
            Stress_due_to_Earth_Load = (Earth_Load_Stiffness_Factor * Earth_Load_Burial_Factor * Earth_Load_Excavation_Factor * Soil_Unit_Weight * Pipe_Outside_Diameter) / 1_000_000
            Stress_due_to_Earth_Load = round(Stress_due_to_Earth_Load, 3)
            print(f"Stress_due_to_Earth_Load (S_He): {Stress_due_to_Earth_Load:.3f} MPa")
            self.lineEdit_35.setText(f"{Stress_due_to_Earth_Load:.3f}") # Update UI for Stress Due to Earth Load (S_He)

            # C) Cyclic Stresses
            Cyclic_Circumferential_Stress = (Stiffness_Factor_KHh * Geometry_Factor_GHh * Road_Pavement_Type_Factor * Road_Axle_Configuration_Factor * Impact_Factor * Applied_Design_Surface_Pressure)
            Cyclic_Circumferential_Stress = round(Cyclic_Circumferential_Stress, 3)
            print(f"Cyclic_Circumferential_Stress (ΔS_Hh): {Cyclic_Circumferential_Stress:.3f} MPa")
            self.lineEdit_34.setText(f"{Cyclic_Circumferential_Stress:.3f}") # Update UI for Cyclic Circumferential Stress (ΔS_Hh)

            Cyclic_Longitudinal_Stress = (Stiffness_Factor_KLh * Geometry_Factor_GLh * Road_Pavement_Type_Factor * Road_Axle_Configuration_Factor * Impact_Factor * Applied_Design_Surface_Pressure)
            Cyclic_Longitudinal_Stress = round(Cyclic_Longitudinal_Stress, 3)
            print(f"Cyclic_Longitudinal_Stress (ΔS_Lh): {Cyclic_Longitudinal_Stress:.3f} MPa")
            self.lineEdit_36.setText(f"{Cyclic_Longitudinal_Stress:.3f}") # Update UI for Cyclic Longitudinal Stress (ΔS_Lh)
            ...
            print(f"Circumferential Stress (S1): {Circumferential:.3f} MPa")
            print(f"Longitudinal Stress (S2): {Longitudinal:.3f} MPa")
            print(f"Radial Stress (S3): {Radial_Stress:.3f} MPa") # This will always be 0.0
            self.lineEdit_38.setText(f"{Circumferential:.3f}") # Update UI for Circumferential Stress (S1)
            self.lineEdit_39.setText(f"{Longitudinal:.3f}") # Update UI for Longitudinal Stress (S2)
            self.lineEdit_30.setText(f"{Radial_Stress:.3f}") # Update UI for Radial Stress (S3)
            ...
            effective_stress = math.sqrt(0.5 * ((Circumferential - Longitudinal)**2 + \
                                                (Longitudinal - Radial_Stress)**2 + \
                                                (Radial_Stress - Circumferential)**2))
            print(f"Effective Stress (S_eff): {effective_stress:.3f} MPa")
            self.lineEdit_40.setText(f"{effective_stress:.3f}") # Update UI for Effective Stress (S_eff)

            # Principle Stress Criteria Check
            if effective_stress <= Specified_Minimum_Yield_Strength * Design_Factor:
                print("Principle Stress Criteria Check: SAFE")
                self.radioButton_5.setChecked(True) # SAFE
                self.radioButton_6.setChecked(False)
            else:
                print("Principle Stress Criteria Check: NOT SAFE")
                self.radioButton_5.setChecked(False)
                self.radioButton_6.setChecked(True) # NOT SAFE

            # F) Fatigue Check (Girth Welds)
            print(f"Fatigue Endurance Limit Of Girth Yield (SFG): {Fatigue_Endurance_Limit_of_Girth_yield} MPa")
            if Cyclic_Longitudinal_Stress <= Fatigue_Endurance_Limit_of_Girth_yield * Design_Factor:
            ...
            # F) Fatigue Check (Longitudinal Welds)
            print(f"Fatigue Endurance Limit Of Longitudinal Weld (SFL): {Fatigue_Endurance_Limit_of_Longitudinal_Weld} MPa")
            if Cyclic_Circumferential_Stress <= Fatigue_Endurance_Limit_of_Longitudinal_Weld * Design_Factor:
